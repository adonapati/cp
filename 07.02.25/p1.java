...
class Solution {
    public static int performance(int n, int[] tasks, int[] hours, int k, int lower, int upper, int work_goal) {
        int t=0, e=0;
        int points=0;
        int i=0, j=0;
        while(j<n) {
            ...
            if(j-i+1<k) {
                ...
                if(t<lower&&e>=work_goal) points-=2;
                if(t<lower&&e<work_goal) points-=1;
                if(t>=upper&&e>=work_goal) points+=1;
                if(t>=upper&&e<work_goal) points+=2;
                t-=tasks[i];
                e-=hours[i];
                i++;
                j++;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(performance(n, tasks, hours, k, lower, upper, work_goal));
        sc.close();
    }
}