...
class Solution {
    public static int build(int n, int maxFreq, int maxArea, int roomSizes[]) {
        HashMap<Integer, Integer> freqs = new HashMap<>();
        int l=0, r=0, max=0;
        long sum=0;
        while(r<n) {
            freqs.put(roomSizes[r],freqs.getOrDefault(roomSizes[r],0)+1);
            sum = (long)(sum+roomSizes[r]);
            while((freqs.get(roomSizes[r])>maxFreq || sum>maxArea)&&l<=r) {
                freqs.put(roomSizes[l],freqs.get(roomSizes[l])-1);
                sum = (long)(sum-roomSizes[l++]);
            }
    ...
    public static void main(String[] args) {
        ...
        int roomSizes[] = new int[n];
        for(int i=0;i<n;i++) {
            roomSizes[i] = sc.nextInt();
        }
        System.out.println(build(n, maxFreq, maxArea, roomSizes));
        sc.close();
    }
}