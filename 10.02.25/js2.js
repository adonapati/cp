...
function fetchAvailableMovies() {
  ...
  async function processBooking(userId) {
    ...
    const [movieFetched, userVerified] = await Promise.all([
    ...
    // First Concurrent Activities: Fetch available movies and check user details
      const movieChosen = await chooseMovie();
      console.log(movieChosen);
      const showtimeChosen = await selectShowtime();
      console.log(showtimeChosen);
