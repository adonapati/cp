...
class Solution {
    public static int marathon(int n, int k, int m, int[] movie, int[] res) {
        Set<Integer> restricted = new HashSet<>();
        for (int rating : res) {
            restricted.add(rating);
        }
        int max = -1;
        for (int i = 0; i <= n - k; i++) {
            ...
            for (int j = i; j < i + k; j++) {
                if (restricted.contains(movie[j]) || !unique.add(movie[j])) {
                    isvalid = false;
                    break;
                }
                curr += movie[j];
            }
            if(isvalid) max = Math.max(max, curr);
        }
        return max;
    }
